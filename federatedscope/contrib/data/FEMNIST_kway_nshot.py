...
import federatedscope.contrib.data.femnist_fedproto_official as femnist
"""
...
def load_data_from_file(config, client_cfgs=None):
    ...
    train_dataset = femnist.FEMNIST(config, file_path, train=True, download=True,
                                    transform=apply_transform)
    test_dataset = femnist.FEMNIST(config, file_path, train=False, download=True,
                                   transform=apply_transform)
    ...
    if config.fedproto.iid:
        ...
        user_groups = femnist_iid(train_dataset, config.federate.client_num)
    else:
        # Sample Non-IID user data from Mnist
        if config.fedproto.unequal:
            user_groups = femnist_noniid_unequal(config, train_dataset, config.federate.client_num)
        else:
            # Chose euqal splits for every user
            user_groups, classes_list, classes_list_gt = femnist_noniid(config, config.federate.client_num, n_list, k_list)
            user_groups_lt = femnist_noniid_lt(config, config.federate.client_num, classes_list)
...
def femnist_noniid(args, num_users, n_list, k_list):
    ...
    for i in range(num_users):
        ...
        classes = random.sample(range(0, args.federate.client_num), n)
...
def call_file_data(config, client_cfgs):
    if config.data.type == "femnist_fedproto":
...
register_data("femnist_fedproto", call_file_data)