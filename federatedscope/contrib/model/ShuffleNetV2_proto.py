...
class ShuffleNetV2_proto(nn.Module):
    def __init__(self, num_classes, save_activations=False):
        super(ShuffleNetV2_proto, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.save_activations:
            self.activations = [out1, out2, out3]
        return out,x1



def call_shufflenetv2_proto(model_config, local_data):
    if 'shufflenetv2_proto' in model_config.type:
        model = ShuffleNetV2_proto(num_classes=model_config.out_channels)
        return model

register_model('shufflenetv2_proto', call_shufflenetv2_proto)
