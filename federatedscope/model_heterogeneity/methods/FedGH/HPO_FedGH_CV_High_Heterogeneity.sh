...
result_floder=model_heterogeneity/result/hpo_0810
...
server_optimizer=('Adam' 'SGD')
server_lr=(0.01 0.001 0.0001)
temp=0
...
train_model() {
  python main.py --cfg ${main_cfg} --client_cfg ${client_file} \
    ...
    eval.freq ${freq} \
    FedGH.server_optimizer.type ${5} \
    FedGH.server_optimizer.lr ${6} \
    MHFL.task ${task}
}
  ...
  for opt in "${optimizer[@]}"; do
    for lr in "${lrs[@]}"; do
      for ls in "${local_update_step[@]}"; do
        for s_op in "${server_optimizer[@]}"; do
          for s_lr in "${server_lr[@]}"; do
            for s in "${seed[@]}"; do
              splitter_args="data.splitter_args ""[{'alpha':${alpha}}]"
              train_model "$s" "$ls" "$opt" "$lr" "$s_op" "$s_lr"
            done
          done
        done
      done
